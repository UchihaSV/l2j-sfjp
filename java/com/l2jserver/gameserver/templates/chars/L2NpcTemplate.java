...
public final class L2NpcTemplate extends L2CharTemplate
{
	...
	public L2NpcTemplate(StatsSet set)
	{
		...
		type = intern(set.getString("type"));
		name = intern(set.getString("name"));
		serverSideName = set.getBool("serverSideName");
		title = intern(set.getString("title"));
		isQuestMonster = title.equalsIgnoreCase("Quest Monster") || title.equals("クエストモンスター");	// [L2J_JP EDIT - TSL][JOJO]
		serverSideTitle = set.getBool("serverSideTitle");
		sex = intern(set.getString("sex"));
		...
		//_npcStatsSet = set;
		_teachInfo = null;
		jClass = intern(set.getString("jClass"));
