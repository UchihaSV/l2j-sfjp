	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (condition == COND_REGULAR)
		{
			...
			else if (actualCommand.equalsIgnoreCase("bidding"))
			{
				...
				try
				{
					...
					if (a != null)
					{
						...
						html.replace("%AGIT_LEASE%", formatAdena(ClanHallManager.getInstance().getClanHallById(a.getItemId()).getLease()));
			...
			else if (actualCommand.equalsIgnoreCase("selectedItems"))
			{
				...
				else if (player.getClan() != null && player.getClan().getHasHideout() != 0)
				{
					...
					if (ClanHallManager.getInstance().getClanHallById(ItemId) != null)
					{
						...
						html.replace("%AGIT_LEASE%", formatAdena(ClanHallManager.getInstance().getClanHallById(ItemId).getLease()));
			...
			else if (actualCommand.equalsIgnoreCase("sale2"))
			{
				...
				NpcHtmlMessage html = new NpcHtmlMessage(1);
				html.setFile(player.getHtmlPrefix(), filename);
				html.replace("%AGIT_LAST_PRICE%", formatAdena(ClanHallManager.getInstance().getClanHallByOwner(player.getClan()).getLease()));
