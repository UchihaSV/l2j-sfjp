	...
	public void showChatWindow(L2PcInstance playerInstance, int val)
	{
		...
		if (TvTEvent.isParticipating())
		{
			...
			if (htmContent != null)
			{
				...
				npcHtmlMessage.replace("%team1playercount%", teamsPlayerCounts[0]);
				npcHtmlMessage.replace("%team2name%", Config.TVT_EVENT_TEAM_2_NAME);
				npcHtmlMessage.replace("%team2playercount%", teamsPlayerCounts[1]);
				npcHtmlMessage.replace("%playercount%", teamsPlayerCounts[0] + teamsPlayerCounts[1]);
				if (!isParticipant)
				{
		...
		else if (TvTEvent.isStarting() || TvTEvent.isStarted())
		{
			...
			if (htmContent != null)
			{
				...
				npcHtmlMessage.replace("%team1name%", Config.TVT_EVENT_TEAM_1_NAME);
				npcHtmlMessage.replace("%team1playercount%", teamsPlayerCounts[0]);
				npcHtmlMessage.replace("%team1points%", teamsPointsCounts[0]);
				npcHtmlMessage.replace("%team2name%", Config.TVT_EVENT_TEAM_2_NAME);
				npcHtmlMessage.replace("%team2playercount%", teamsPlayerCounts[1]);
				npcHtmlMessage.replace("%team2points%", teamsPointsCounts[1]); // <---- array index from 0 to 1 thx DaRkRaGe
				playerInstance.sendPacket(npcHtmlMessage);
			}
		}
