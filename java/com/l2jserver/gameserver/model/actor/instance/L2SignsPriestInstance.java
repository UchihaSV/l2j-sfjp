		...
		else if (command.startsWith("SevenSigns"))
		{
			...
			switch (val)
			{
				...
				case 20: // Seal Status (for when joining a cabal)
					StringBuilder contentBuffer = new StringBuilder(2000);	//+[JOJO]
				//	TextBuilder contentBuffer = new TextBuilder();
					...
					for (int i = 1; i < 4; i++)
					{
						...
						{
							contentBuffer.append("[" + SevenSigns.getSealNameJA(i, false) + ": " + SevenSigns.getCabalName(sealOwner) + "]<br>");
						}
						else
						{
							contentBuffer.append("[" + SevenSigns.getSealNameJA(i, false) + ": Nothingness]<br>");
						}
					}
					
					contentBuffer.append("<a action=\"bypass -h npc_" + getObjectId() + "_Chat 0\">–ß‚é</a></body></html>");
