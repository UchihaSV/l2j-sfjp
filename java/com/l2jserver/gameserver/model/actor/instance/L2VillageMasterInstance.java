...
public class L2VillageMasterInstance extends L2NpcInstance
{
	private static Logger _log = Logger.getLogger(L2VillageMasterInstance.class.getName());
  	...
  	private static final boolean ALLOW_BASECLASS_EVERYWHERE = false;	//[JOJO]
	...
	{
		return getHtmlPath("data/html/villagemaster/", npcId, val);
	/*	String pom = "";
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (actualCommand.equalsIgnoreCase("change_clan_leader"))
		{
			...
			if (!member.isOnline())
			{
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_IS_NOT_ONLINE).addString(cmdParams));	//+[JOJO]
		...
		{
			if (player.isCursedWeaponEquipped())	// [L2J_JP ADD - TSL]
			{
				player.sendMessage("呪われた武器を装着した状態ではサブ クラスを作ることや、変更することはできません。");
			//	player.sendMessage("In the state with the CursedSword, it is not possible to become a subclass.");
				return;
			}
...
//					if (player.isSubClassActive())	// [L2J_JP ADD - TSL]
//					{
//						player.sendMessage("サブ クラスの状態では使用できません。メイン クラスに変更してからもう一度行ってください。");
//					//	player.sendMessage("You must change your main class.");
//						return;
//					}
				...
				case 2: // Change Class - Initial
					...
					else
					{
						...
						//[JOJO]
						if ((ALLOW_BASECLASS_EVERYWHERE || checkVillageMaster(player.getBaseClass()))
								&& player.getActiveClass() != player.getBaseClass())
						{
