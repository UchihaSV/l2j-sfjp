...
public final class L2PcInstance extends L2Playable
{
	...
	private static final String UPDATE_CHARACTER = "UPDATE characters SET level=?,maxHp=?,curHp=?,maxCp=?,curCp=?,maxMp=?,curMp=?,face=?,hairStyle=?,hairColor=?,sex=?,heading=?,x=?,y=?,z=?,exp=?,expBeforeDeath=?,sp=?,karma=?,fame=?,pvpkills=?,pkkills=?,clanid=?,race=?,classid=?,deletetime=?,title=?,title_color=?,accesslevel=?,online=?,isin7sdungeon=?,clan_privs=?,wantspeace=?,base_class=?,onlinetime=?,newbie=?,nobless=?,power_grade=?,subpledge=?,lvl_joined_academy=?,apprentice=?,sponsor=?,clan_join_expiry_time=?,clan_create_expiry_time=?,char_name=?,death_penalty_level=?,bookmarkslot=?,vitality_points=?,language=?,hero=? WHERE charId=?";		//[L2J_JP EDIT] <,hero=?> 追加
	...
	private final FastList<IEventListener> _eventListeners = new FastList<IEventListener>().shared();
 ...
 //	public L2AccessLevel getAccessLevel()
	...
	private void storeCharBase()
	{
			...
			PreparedStatement statement = con.prepareStatement(UPDATE_CHARACTER))
		{
			...
			statement.setInt(50, (isHero() ? 1 : 0));	//+[L2J_JP ADD]
			statement.setInt(51, getObjectId());		// [L2J_JP EDIT] 番号を１つ増やすこと
...
//		}
	...
	public boolean addSubClass(int classId, int classIndex)
	{
		...
		try
		{
			...
			final HashMap<Integer, Skill> prevSkillList = new HashMap<>();
	...
	public final void onTeleported()
	{
		...
		if (summon != null)
		{
			...
			summon.teleToLocation(getPosition(), false);
	...
	{
		L2SkillLearn learn;
		for (Skill skill : getSkills().values())	//[JOJO] -entrySet()
		{
	...
	//[JOJO]-------------------------------------------------
	// L2Character から L2PcInstance へ引越し.
