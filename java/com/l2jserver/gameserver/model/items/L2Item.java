...
public abstract class L2Item implements IIdentifiable
{
	...
	protected L2Item(StatsSet set)
	{
		...
		_icon = intern(set.getString("icon", null));
	 ...
	 *         (!skill.checkCondition(caster, target, true)) continue; // Skill condition not met if (target.getFirstEffect(skill.getId()) != null) target.removeEffect(target.getFirstEffect(skill.getId())); for (L2Effect e : skill.applyEffects(caster, target)) effects.add(e); } if (effects.isEmpty())
