...
public class L2BossZone extends L2ZoneType
{
 ...
 //	private int _timeInvade;			//-[JOJO]
	...
	// of server restart
	private FastSet/*L2FastList*/<Integer> _playersAllowed;	//[JOJO]
		...
		if (_enabled)
		{
			if (character instanceof L2PcInstance)
			{
				...
				if (_playersAllowed.contains(player.getObjectId()))
				{
					...
					_playersAllowed.remove(player.getObjectId());							//+[JOJO] FastSet
				//	_playersAllowed.remove(_playersAllowed.indexOf(player.getObjectId()));	//-[JOJO] FastList
				}
		...
		if (_enabled)
		{
			if (character instanceof L2PcInstance)
			{
				...
				if (!player.isOnline() && _playersAllowed.contains(player.getObjectId()))
				{
					...
					_playerAllowedReEntryTimes.put(player.getObjectId(), System.currentTimeMillis() + _timeClientDown/*_timeInvade*/);
				}
				else
				{
					_playersAllowed.remove(player.getObjectId());								//+[JOJO] FastSet
				//	if (_playersAllowed.contains(player.getObjectId()))							//-[JOJO] FastList
				//		_playersAllowed.remove(_playersAllowed.indexOf(player.getObjectId()));	//-[JOJO] FastList
					_playerAllowedReEntryTimes.remove(player.getObjectId());
				}
			}
	...
	public void setAllowedPlayers(FastSet/*L2FastList*/<Integer> players)
	{
		if (players != null)
			_playersAllowed = players;
	}
	
	public FastSet/*L2FastList*/<Integer> getAllowedPlayers()
	{
