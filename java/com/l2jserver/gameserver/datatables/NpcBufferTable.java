...
public class NpcBufferTable
{
	...
	private static class NpcBufferSkills
	{
		...
		public int[] getSkillGroupInfo(int buffGroup)	//[JOJO] (TIntIntHashMapópÇ…èëÇ´ä∑Ç¶)
		{
			int skillId = _skillId.get(buffGroup);
			int skillLevel = _skillLevels.get(buffGroup);
			int skillFeeId = _skillFeeIds.get(buffGroup);
			int skillFeeAmount = _skillFeeAmounts.get(buffGroup);
