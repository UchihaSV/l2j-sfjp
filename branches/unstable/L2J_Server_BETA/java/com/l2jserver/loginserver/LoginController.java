...
public class LoginController
{
	...
	private final ConcurrentFastMap<InetAddress, Long> _bannedIps = new ConcurrentFastMap<>();	//[JOJO] -FastMap.shared
	...
	{
		_loginServerIpAddrs.remove(account);
	}

	public String getLoginIpAddr(String account)
	{
		return _loginServerIpAddrs.get(account);
	}
	...
	{
		if (!Config.DENY_MULTI_LOGIN_BY_IPADDR) return false;
		if (!_loginServerIpAddrs.containsKey(account) && _loginServerIpAddrs.containsValue(address))
		{
			if (getUserLevel(account) > 0)
				return false;
			else
				return true;
		}
	...
	public AuthLoginResult tryCheckinAccount(L2LoginClient client, InetAddress address, AccountInfo info, LoginFailReason[] loginFailReason)	//[JOJO]
	{
		...
		if (Config.DENY_BANNED_USER_BY_IPADDR && isIPinBanList(info.getLogin(), ip))
			return AuthLoginResult.ACCOUNT_BANNED;
		// [L2J_JP ADD END - TSL]

		// check auth
		if (canCheckin(client, address, info, loginFailReason))
		{
			...
			if (!isAccountInAnyGameServer(info.getLogin()))
			{
				...
				synchronized (_loginServerIpAddrs)
				{
	 ...
	 */
	public boolean canCheckin(L2LoginClient client, InetAddress address, AccountInfo info, LoginFailReason[] loginFailReason)	//[JOJO]
	{
		loginFailReason[0] = LoginFailReason.REASON_USER_OR_PASS_WRONG;
		try
		{
	...
	public static enum AuthLoginResult
	{
		INVALID_PASSWORD,
		ACCOUNT_BANNED,
		ALREADY_IPADDR,		// [L2J_JP EDIT - TSL]
		ALREADY_ON_LS,
		ALREADY_ON_GS,
		AUTH_SUCCESS
	}
