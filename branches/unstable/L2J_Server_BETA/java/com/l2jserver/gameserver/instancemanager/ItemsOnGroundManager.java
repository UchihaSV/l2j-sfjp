...
public class ItemsOnGroundManager
{
	...
	public void save(L2ItemInstance item)
	{
		...
		if (item.getInstanceId() != 0)
			return;
		//-------------------------------------------------------
		synchronized (_items) { _items.add(item); }	//[JOJO]ëÑÅEîÕàÕéÎëŒçÙ
	}
	...
	public void removeObject(L2ItemInstance item)
	{
		...
		{
			synchronized (_items) { _items.remove(item); }	//[JOJO]ëÑÅEîÕàÕéÎëŒçÙ
		}
	}
	
	private L2ItemInstance[] itemstoArray()
	{
		synchronized (_items) { return _items.toArray(new L2ItemInstance[_items.size()]); }	//[JOJO]ëÑÅEîÕàÕéÎëŒçÙ
	}
