...
public final class MapRegionManager extends DocumentParser
{
			...
			if ("list".equalsIgnoreCase(n.getNodeName()))
			{
				for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
				{
					if ("region".equalsIgnoreCase(d.getNodeName()))
					{
						...
						int locId = parseInteger(attrs, "locId");
						int castle = parseInteger(attrs, "castle");
						int bbs = parseInteger(attrs, "bbs");
	...
	{
		return getMapRegion(x, y);
	}
	
	/**
	 * Get town name by coordinate
	 * @param x, y
	 * @return String
	 */
	public String getClosestTownName(int x, int y)
	{
		L2MapRegion region = getMapRegion(x, y);
