...
public class FourSepulchersManager
{
 ...
 //	protected TIntObjectHashMap<L2Spawn> _keyBoxSpawns = new TIntObjectHashMap<L2Spawn>();		//-[JOJO]
	...
	protected void timeCalculator()
	{
		...
		tmp.set(Calendar.SECOND, 0);		//+[JOJO] ç≈ëÂÇPï™ÇÃåÎç∑Ç™èoÇƒÇµÇ‹Ç§ÇÃÇ≈
	...
	public void managerSay(byte min)
	{
		...
		if (_inAttackTime)
		{
			...
			String msg = /*TODO:1000455*/"åªç›" + min + /*TODO:1000456*/"ï™Ç™åoâﬂÇµÇ‹ÇµÇΩ"; // now this is a
		...
		{
			final int msg1 = 1000500; //"ë¸ç°ÇÊÇËóÏï_ì‡Ç÷ì¸ÇÈÇ±Ç∆Ç™Ç≈Ç´Ç‹Ç∑ÅB"
			final int msg2 = 1000501; //"élëÂóÏï_ÇÃêŒëúÇ…éËÇèÊÇπÇÈÇ∆óÏï_ì‡Ç…ì¸ÇÈÇ±Ç∆Ç™Ç≈Ç´Ç‹Ç∑ÅB"
			...
			if (_firstTimeRun)
			{
				for (double min = Calendar.getInstance().get(Calendar.MINUTE); min < _newCycleMin; min++)
				{
					...
					//	_log.info(Calendar.getInstance().getTime() + " Atk announce scheduled to " + min + " minute of this hour.");
						...
						inter.set(Calendar.SECOND, 0);		//+[JOJO] ç≈ëÂÇPï™ÇÃåÎç∑Ç™èoÇƒÇµÇ‹Ç§ÇÃÇ≈
