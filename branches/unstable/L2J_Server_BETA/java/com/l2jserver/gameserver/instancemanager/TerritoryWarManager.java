...
public final class TerritoryWarManager implements Siegable
{
	...
	public void giveTWQuestPoint(L2PcInstance player)
	{
		...
		{
			_participantPoints.put(player.getObjectId(), new int[]
			{
				player.getSiegeSide(),
				0,
				0,
	...
	public void giveTWPoint(L2PcInstance killer, int victimSide, int type)
	{
		...
		if ((killer.getParty() != null) && (type < 5))
		{
			for (L2PcInstance pl : killer.getParty().getMembers())
			{
				...
				{
					_participantPoints.put(pl.getObjectId(), new int[]
					{
						pl.getSiegeSide(),
						0,
						0,
		...
		else
		{
			...
			{
				_participantPoints.put(killer.getObjectId(), new int[]
				{
					killer.getSiegeSide(),
					0,
					0,
	...
	protected class ScheduleStartTWTask implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				long timeRemaining = _startTWDate.getTimeInMillis() - System.currentTimeMillis();
	...
	private class ScheduleEndTWTask implements Runnable
	{
		...
		public void run()
		{
			...
			{
				_scheduledEndTWTask.cancel(false);
				long timeRemaining = (_startTWDate.getTimeInMillis() + WARLENGTH) - System.currentTimeMillis();
