...
if (com.l2jserver.Config.FIX_ATTACKABLE_AI_FACTION_CALL) {{
						if (npc == called)
							continue;
						if (!Util.checkIfInShortRadius(npc.getFactionRange(), npc, called, false))
							continue;
...
//									&& GeoData.getInstance().canSeeTarget(called, npc))
		...
		if (!npc.isMovementDisabled() && npc.getCanDodge() > 0)
			if (Rnd.get(100) <= npc.getCanDodge())
			{
				...
				{
					int posX = _actor.getX() + Rnd.get(-100, 100);
					int posY = _actor.getY() + Rnd.get(-100, 100);
					int posZ = _actor.getZ() + 30;
					/*
					int posX = npc.getX();
					int posY = npc.getY();
