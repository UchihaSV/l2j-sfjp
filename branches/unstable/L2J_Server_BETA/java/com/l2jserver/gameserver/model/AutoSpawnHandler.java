...
public class AutoSpawnHandler
{
	...
	private static final boolean TEST1 = true;	//[JOJO] getNPCInstanceList() で "Destination array too small" が出る問題.サーバ機の処理能力に依存しないように.
	...
	private class AutoSpawner implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				final long now = System.currentTimeMillis();	//+[JOJO]
				final boolean isDespawnSchedule = spawnInst._desDelay < spawnInst._resDelay;	//+[JOJO]
if (TEST1) {{
				if (spawnInst._desDelay > 0 && !isDespawnSchedule)	//+[JOJO]
					new AutoDespawner(_objectId).run();
}}
				...
				if (spawnInst.isBroadcasting() && (npcInst != null))
				{
					Announcements.getInstance().announceToAll( SystemMessage.getSystemMessage(SystemMessageId.S2_S1)
								.addString("に" + npcInst.getName() + "がやってまいりました！")
...
if (TEST1) {{
				if (spawnInst._desDelay > 0 && isDespawnSchedule)
				{
					...
					ThreadPoolManager.getInstance().scheduleAi(rd, now + spawnInst._desDelay - System.currentTimeMillis());
				}
}} else {{
				if (spawnInst._desDelay > 0)
				{
					AutoDespawner rd = new AutoDespawner(_objectId);
					ThreadPoolManager.getInstance().scheduleAi(rd, spawnInst._desDelay - 1000);
				}
}}
			}
			catch (Exception e)
	...
	//	}
		...
		{
			return _npcList.toArray(new L2Npc[_npcList.size()]);
	//		L2Npc[] ret;
	//		synchronized (_npcList)
	//		{
	//			ret = new L2Npc[_npcList.size()];
	//			_npcList.toArray(ret);	//TODO:java.lang.UnsupportedOperationException: Destination array too small (2006/11/**)
	//		}
	//		
	//		return ret;
		}
