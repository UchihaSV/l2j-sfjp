...
public final class L2PcInstance extends L2Playable
{
	...
	private static final String UPDATE_CHARACTER = "UPDATE characters SET level=?,maxHp=?,curHp=?,maxCp=?,curCp=?,maxMp=?,curMp=?,face=?,hairStyle=?,hairColor=?,sex=?,heading=?,x=?,y=?,z=?,exp=?,expBeforeDeath=?,sp=?,karma=?,fame=?,pvpkills=?,pkkills=?,clanid=?,race=?,classid=?,deletetime=?,title=?,title_color=?,accesslevel=?,online=?,isin7sdungeon=?,clan_privs=?,wantspeace=?,base_class=?,onlinetime=?,punish_level=?,punish_timer=?,newbie=?,nobless=?,power_grade=?,subpledge=?,lvl_joined_academy=?,apprentice=?,sponsor=?,clan_join_expiry_time=?,clan_create_expiry_time=?,char_name=?,death_penalty_level=?,bookmarkslot=?,vitality_points=?,language=?,hero=? WHERE charId=?";		//[L2J_JP EDIT] <,hero=?> 追加
 ...
 //	public L2AccessLevel getAccessLevel()
	...
	//[JOJO]-------------------------------------------------
	// L2Chracter から l2pcInstance へ引越し.
	...
	private void storeCharBase()
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			statement.setInt(52, (isHero() ? 1 : 0));	//+[L2J_JP ADD]
			statement.setInt(53, getObjectId());		// [L2J_JP EDIT] 番号を１つ増やすこと
	...
	private boolean checkUseMagicConditions(L2Skill skill, boolean forceUse, boolean dontMove)
	{
		...
		if (target.isDoor())
		{
			...
			Castle doorCastle;
			Fort doorFort;
			if (((doorCastle = door.getCastle()) != null) && (doorCastle.getCastleId() > 0)) // If its castle door
			{
			...
			else if (((doorFort = door.getFort()) != null) && (doorFort.getFortId() > 0 && !door.getIsShowHp())) // If its fort door
			{
