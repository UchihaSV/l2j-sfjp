...
public final class L2PcInstance extends L2Playable
{
	...
	{
		NONE(0, ""),
		CHAT(1, "チャット禁止"/*"chat banned"*/),
		JAIL(2, "GM相談所"/*"jailed"*/),
		CHAR(3, "キャラクター凍結"/*"banned"*/),
		ACC(4, "アカウント凍結"/*"banned"*/);
 ...
 //		_radar = new L2Radar(this);
	...
	public void setTemplate(ClassId newclass) { super.setTemplate(CharTemplateTable.getInstance().getTemplate(newclass, getAppearance().getSex())); }	// [L2J_JP EDIT - TSL]
	...
	public Race getRace()
	{
		...
		L2PcTemplate charTemp = CharTemplateTable.getInstance().getTemplate(_baseClass, getAppearance().getSex());	// [L2J_JP EDIT - TSL]
		return charTemp.race;
	}
	
	public L2Radar getRadar()
	{
		return new L2Radar(this);	//[JOJO]
 //		return _radar;
	}
	...
	public void sitDown(boolean checkCast)
	{
		...
		{
		//	sendMessage("Cannot sit while casting");	//[L2J_JP DELETE]
			return;
		}
	...
	public boolean destroyItem(String process, L2ItemInstance item, long count, L2Object reference, boolean sendMessage)
	{
		...
		if (item == null)
		{
			if (sendMessage)
			{
				sendPacket(SystemMessage.getSystemMessage(SystemMessageId.NOT_ENOUGH_REQUIRED_ITEMS));	// [L2J_JP EDIT]
			}
			return false;
		}
		...
		if (sendMessage)
		{
			if (item.getItemId() == 57)												//+[JOJO]
				sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_DISAPPEARED_ADENA)		//+[JOJO]
							.addItemNumber(count));									//+[JOJO]
			else																	//+[JOJO]
			if (count > 1)
			{
				SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.S2_S1_DISAPPEARED);
	...
	public boolean dropItem(String process, L2ItemInstance item, L2Object reference, boolean sendMessage, boolean protectItem)
	{
		...
		{
			if (sendMessage)
				sendPacket(SystemMessage.getSystemMessage(SystemMessageId.ITEM_NOT_DISCARDED));	// [L2J_JP EDIT]
		...
		if (Config.DESTROY_DROPPED_PLAYER_ITEM) {
			if (item.isStackable() && item.getCount() < 2)	//+[JOJO]
			 	item.setProtected(false);					//+[JOJO]
			else											//+[JOJO]
			if (!item.isEquipable()
		...
		if (checkLandingState() && getMountType() == 2)
		{
			if (!Config.ALT_WYVERN_MAKEALANDING) //Wyvern can make a landing in zone of 'No Landing'
			{
				if (!isGM() && Config.ALT_DISMOUNT_WYVERN_IN_NOLANDING) //Playe moves to the nearest town.
					teleToLocation(MapRegionTable.getInstance().getTeleToLocation(this, MapRegionTable.TeleportWhereType.Town), true);
				else
					return false;
			}
		}
 ...
 //	public L2AccessLevel getAccessLevel()
	...
	private boolean createDb()
	{
		...
		try
		{
			...
			statement.setInt(28, getL2AccessLevel/*getAccessLevel*/().getLevel());	//[JOJO]
	...
	private void storeCharBase()
	{
		...
		try
		{
			...
			statement.setInt(29, getL2AccessLevel()/*getAccessLevel()*/.getLevel());	//[JOJO]
			...
			statement.setInt(53, (isHero() ? 1 : 0));	//+[L2J_JP ADD]
			statement.setInt(54, getObjectId());		// [L2J_JP EDIT] 番号を１つ増やすこと
	...
	public boolean disableAutoShot(int itemId)
	{
		if (_activeSoulShots.contains(itemId))
		{
			...
			sendPacket(SystemMessage.getSystemMessage(SystemMessageId.AUTO_USE_OF_S1_CANCELLED)	//[JOJO]
					.addItemName(itemId));	//[JOJO]
			return true;
		}
		else return false;
	}
...
//		}
	...
	private void setClassTemplate(int classId)
	{
		...
		L2PcTemplate t = CharTemplateTable.getInstance().getTemplate(classId, getAppearance().getSex());	// [L2J_JP EDIT - TSL]
		...
		switch (state)
		{
			...
			case JAIL: // Jail Player
			{
				...
				if (delayInMinutes > 0)
				{
					...
					// start the countdown
					_punishTask = ThreadPoolManager.getInstance().scheduleGeneral(new PunishTask(), _punishTimer);
					sendMessage(delayInMinutes+"分間GM相談所に収容されます。");
	...
	private void updatePunishState()
	{
		if (getPunishLevel() != PunishLevel.NONE)
		{
			...
			{
				_punishTask = ThreadPoolManager.getInstance().scheduleGeneral(new PunishTask(), _punishTimer);
				sendMessage(Math.round(_punishTimer/60000)+"分間の"+getPunishLevel().string()+"中です。");	//[JOJO]
	...
	//[JOJO]-------------------------------------------------
	/**
	 * 一時的に経験値とSPの取得をON/OFFするマスク
	 * チャットコマンド 「.xpon」「.xpoff」で切り替える。
