...
public class Siege implements Siegable
{
	...
	public void endSiege()
	{
		if (getIsInProgress())
		{
			SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.SIEGE_OF_S1_HAS_ENDED);	//$s1‚ÌUéí‚ªI—¹‚µ‚Ü‚µ‚½B
			sm.addFortId(getCastle().getCastleId());	//[JOJO]
			Announcements.getInstance().announceToAll(sm);
			...
			if (getCastle().getOwnerId() > 0)
			{
				...
				sm = SystemMessage.getSystemMessage(SystemMessageId.CLAN_S1_VICTORIOUS_OVER_S2_S_SIEGE);	//$s1 ŒŒ–¿‚ª $s2 ‚Æ‚ÌUéí‚ÅŸ—˜‚µ‚Ü‚µ‚½B
			...
			{
				sm = SystemMessage.getSystemMessage(SystemMessageId.SIEGE_S1_DRAW);	//$s1‚Ìè—Ìí‚ªˆø‚«•ª‚¯‚ÅI—¹‚µ‚Ü‚µ‚½B
				sm.addFortId(getCastle().getCastleId());	//[JOJO]
				Announcements.getInstance().announceToAll(sm);
			}
	...
	public void startSiege()
	{
		if (!getIsInProgress())
		{
			...
			if (getAttackerClans().isEmpty())
			{
				...
				sm.addFortId(getCastle().getCastleId()); // [L2J_JP EDIT - TSL][JOJO]
				Announcements.getInstance().announceToAll(sm);
				saveCastleSiege();
				return;
			}
			...
			SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.SIEGE_OF_S1_HAS_STARTED);	//$s1‚ÌUéí‚ªŽn‚Ü‚è‚Ü‚µ‚½B
			sm.addFortId(getCastle().getCastleId());	//[JOJO]
			Announcements.getInstance().announceToAll(sm);
		}
	}
	...
	public void startAutoTask()
	{
		...
		_log.info("Siege of " + getCastle().getCastleName() + ": " + com.l2jserver.util.Util.dateFormat(getCastle().getSiegeDate()));
