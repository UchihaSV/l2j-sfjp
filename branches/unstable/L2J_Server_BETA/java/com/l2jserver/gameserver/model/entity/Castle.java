...
public class Castle
{
	...
	private String _castleName, _locName;	//+[JOJO]
	...
	private void load()
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			...
			PreparedStatement ps1 = con.prepareStatement("SELECT castle.*,castlename_ja.name AS castleName,castlename_ja.locName FROM castle"
			...
			PreparedStatement ps2 = con.prepareStatement("SELECT clan_id FROM clan_data WHERE hasCastle = ?"))
		{
			...
			try (ResultSet rs = ps1.executeQuery())
			{
				while (rs.next())
				{
					...
					//[JOJO]-------------------------------------------------
					_castleName = rs.getString("castleName");
					_locName = rs.getString("locName").intern();
					//-------------------------------------------------------
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			if (_productionNext != null)
			{
				...
				if (values.length > 0)
				{
					...
					try (PreparedStatement ps3 = con.prepareStatement(query.toString()))
					{
						ps3.execute();
					}
				}
