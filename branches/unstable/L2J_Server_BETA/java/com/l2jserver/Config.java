...
public final class Config
{
	...
	public static void load()
	{
		if (Server.serverMode == Server.MODE_GAMESERVER)
		{
			...
			try
			{
				...
				{
					L2Properties settings = new L2Properties();
					is = new FileInputStream(JP_CUSTOM_FILE);
					settings.load(is);
					is.close();
 ...
 //					CRYSTAL_ENCHANT_CHANCE_JEWELRY = Integer.parseInt(settings.getProperty("CrystalEnchantChanceJewelry", "66"));
				...
				try
				{
					...
					is = new FileInputStream(BOSS_FILE);
		...
		else if (Server.serverMode == Server.MODE_LOGINSERVER)
		{
			...
			try
			{
				try
				{
					...
					DENY_MULTI_LOGIN_BY_IPADDR = Boolean.parseBoolean(serverSettings.getProperty("DenyMultiLoginByIpAddr", "false"));	// [L2J_JP ADD - TSL]
					DENY_BANNED_USER_BY_IPADDR = Boolean.parseBoolean(serverSettings.getProperty("DenyBannedUserByIpAddr", "false"));	// [L2J_JP ADD - TSL]
				}
				catch (Exception e)
				{
					_log.warning("Config: " + e.getMessage());
